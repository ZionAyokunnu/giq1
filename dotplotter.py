...
def create_linear_dotplot_with_linearised(genome1_df, genome2_df, genome1_name, genome2_name, output_path, chromosome_sizes_df=None):
    ...
    # EXPLICIT CHANGE: alias expected 'start' column to existing 'gene_start'
    g1_common['start'] = g1_common['gene_start']
    g2_common['start'] = g2_common['gene_start']
    ...
    # Linearised dotplot (center column spans both rows)
    ...
    # Additional chromosome info (bottom right)
...
def linearise_genome_coordinates(genome_df, chromosome_sizes_df=None):
    ...
    if chromosome_sizes_df is not None:
        ...
        for chr_name in sorted_chrs:
            ...
            # EXPLICIT CHANGE: accept either 'chromosome_size_bp' or 'chromsome_size_b'
            if 'chromosome_size_bp' in chromosome_sizes_df.columns:
                chr_size = int(chromosome_sizes_df[chromosome_sizes_df['chromosome'] == chr_name]['chromosome_size_bp'].iloc[0])
            else:
                chr_size = int(chromosome_sizes_df[chromosome_sizes_df['chromosome'] == chr_name]['chromsome_size_b'].iloc[0])
        ...
        # Group by chromosome and find span using gene_start/gene_end
        chr_groups = genome_df.groupby('sequence').agg(start_min=('gene_start','min'),
                                                       end_max=('gene_end','max'))
        sorted_chrs = sorted(chr_groups.index)
        
        for chr_name in sorted_chrs:
            chr_offsets[chr_name] = cumulative_offset
            chr_size = int(chr_groups.loc[chr_name, 'end_max'] - chr_groups.loc[chr_name, 'start_min'] + 10000)  # padding
            cumulative_offset += chr_size
    
    # Add linearised coordinates (expects 'start' already present; we aliased it above)
    genome_linear['linear_position'] = genome_linear.apply(
        lambda row: chr_offsets.get(row['sequence'], 0) + int(row['start']), axis=1
    )
...
def add_chromosome_boundaries(ax, g1_offsets, g2_offsets):
    ...
    for chr_name, offset in sorted(g1_offsets.items(), key=lambda x: x[1]):
    ...
    for chr_name, offset in sorted(g2_offsets.items(), key=lambda x: x[1]):
        if offset > 0:  # Don't add line at position 0
...
def main():
    ...
    try:
        ...
        # EXPLICIT CHANGE: use the new combined function
        stats = create_linear_dotplot_with_linearised(
            genome1_df, genome2_df, args.name1, args.name2, args.output_plot,
            chromosome_sizes_df=None  # optional; pass get_chromosome_sizes_from_sheet() if desired
        )
        
        print("\n" + "="*50)
        print("DOTPLOT COMPARISON COMPLETE")
        print("="*50)
        if isinstance(stats, dict) and 'linear_correlation' in stats:
            print(f"Linear correlation: {stats['linear_correlation']:.3f}")
