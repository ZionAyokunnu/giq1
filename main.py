...
def generate_analysis_report(inversion_df: pd.DataFrame,
   ...
   with open(report_path, 'w') as f:
       ...
       if len(all_inversion_events) > 0:
           ...
           f.write("| Chr Pair | Type | Genes | Start Gene | End Gene | Span (bp) |\n")
           f.write("|----------|------|-------|------------|----------|----------|\n")
           
           for event in sorted(all_inversion_events, key=lambda x: x['gene_count'], reverse=True)[:20]:
               f.write(f"| {event['chr1']}-{event['chr2']} | {event['type']} | {event['gene_count']} | "
                      f"{event['start_gene']} | {event['end_gene']} | {event['span_bp']:,} |\n")
           
           if len(all_inversion_events) > 20:
               f.write(f"\n*Showing top 20 events. Total: {len(all_inversion_events)} events.*\n")
