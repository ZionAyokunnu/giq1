...
Fixed version with proper abstract chromosome IDs and structure.
"""
...
def extract_agora_ancestral_genome_to_busco(agora_file: str, output_tsv_path: str):
    ...
    Uses abstract chromosome IDs (chr1, chr2, etc.) as AGORA intended.
    """
    ...
    for line_num, line in enumerate(lines, 1):
        ...
        if len(parts) >= 6:
            # Format: 0  0  1  1  552100.00.1  Species|BUSCO_ID  Species|BUSCO_ID  ...
            ...
            # Format: 552100.00.1  Species|BUSCO_ID  Species|BUSCO_ID  ...
            agora_chromosome_id = 1  # Default to chromosome 1
        ...
        if busco_ids:
            ...
            # Calculate position within chromosome (reset for each chromosome)
            ...
            ancestral_genes.append({
                'busco_id': busco_id,
                'status': 'Complete',
                'sequence': current_chromosome,  # Use abstract chromosome ID
                ...
                'position_in_chromosome': position_in_chr,
    ...
    # Print summary with abstract chromosome information
    print("\nAGORA Ancestral Genome Summary:")
    print(f"  Total genes: {len(busco_tsv)}")
    print(f"  Abstract chromosomes: {sorted(busco_tsv['sequence'].unique())}")
    ...
    for i, busco_id in enumerate(busco_tsv['busco_id'].head(10)):
        chr_name = busco_tsv.iloc[i]['sequence']
        print(f"  {i+1}. {busco_id} ({chr_name})")
